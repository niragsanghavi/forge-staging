...
function _ctxEntry(ctx){
  ...
  const seasonWindows=tw.filter(w=>w && w.month===month && w.year===year&&(w.groupCode==null||w.groupCode===myGC));
  ...
  const validWindow=w=>Number.isInteger(w.monDate)&&w.monDate>=1&&w.monDate<=DAYS&&Number.isInteger(w.sunDate)&&w.sunDate>=1&&w.sunDate<=31;
  const endOf=w=>Math.min(DAYS,w.sunDate>=w.monDate ? w.sunDate : DAYS);
  ...
  for(const w of seasonWindows){
    if(w.twist!=='boss_week'||!validWindow(w)) continue;
  ...
  for(const w of seasonWindows){
    ...
    // Recipient and period fields are not proof of authority. These guards
    // bound duplicate copies only; server-exclusive settlement is still
    // required to reject fabricated periods or unauthorized winners.
    // Duplicate recipients or duplicate settlement docs must not multiply a
    // round. Keep the highest bounded award, never sum conflicting copies.
    const period=typeof w.week==='string'&&w.week?`week:${w.week}`:validWindow(w)?`range:${w.monDate}-${endOf(w)}`:'legacy-unkeyed';
    const awards=stepByPeriod.get(period)||new Map();
    for(const name of new Set(w.awarded))if(typeof name==='string')awards.set(name,Math.max(awards.get(name)||0,pts));
    stepByPeriod.set(period,awards);
  }
...
function score(playerName, ctx){
  ...
  if(r) return {...r,days:new Set(r.days)};
...
// Solo is a separate scoring lane. This function never reads group globals.
// A workout day earns 4 + min(consecutive days, 3); the run resets each season.
window.scoreSoloDays = function(logs, year, month, throughDay){
  if(!Number.isInteger(year)||year<2000||year>9999||!Number.isInteger(month)||month<1||month>12)throw new Error('INVALID_SOLO_MONTH');
  const count=new Date(Date.UTC(year,month,0)).getUTCDate();
  if(!Number.isInteger(throughDay)||throughDay<0||throughDay>count)throw new Error('INVALID_SOLO_DAY');
  const days=[...new Set((Array.isArray(logs)?logs:[]).filter(l=>l&&!l.voided&&!l.demo&&l.year===year&&l.month===month&&Number.isInteger(l.day)&&l.day>0&&l.day<=throughDay&&Array.isArray(l.workouts)&&l.workouts.some(w=>typeof w==='string'&&w.trim())).map(l=>l.day))].sort((a,b)=>a-b);
  let run=0,previous=0,bonus=0,longest=0;
  const breakdown=days.map(day=>{run=day===previous+1?run+1:1;previous=day;longest=Math.max(longest,run);const streakBonus=Math.min(run,3);bonus+=streakBonus;return {day,base:4,streakBonus,total:4+streakBonus};});
  return {days:days.length,base:days.length*4,bonus,total:days.length*4+bonus,currentStreak:days.length&&previous>=throughDay-1?run:0,longestStreak:longest,breakdown};
};
